...
const DancingOrangeIcon = ({ id, iconSize, isHeavy }) => {
    ...
    return (
        ...
        >
            {/* Viewbox 0 0 120 120 so arms and stars don't get cut off */}
...
return (
    <div key={visual} className="visual-container" style={{ position: 'relative', overflow: 'hidden', minHeight: '300px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '20px', width: '100%', padding: '20px', borderRadius: '20px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'center', gap: flexGap, width: "100%", zIndex: 5 }}>
        
        {/* GROUP A */}
        <AnimatePresence>
          {Array(countA).fill(0).map((_, i) => (
              <motion.div key={`a-${i}`} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: i * appearDelay }} >
                <div className='static-shape' style={{ position: 'relative' }}>
        ...
        {/* OPERATOR */}
        {countB > 0 && (
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: countA * appearDelay }} style={{ margin: '0 10px' }}>
